...
#define HYSegmentedControl_Height 41.0
...
//        self.backgroundColor = [UIColor whiteColor];//UIColorFromRGBValue(0xf3f3f3);
        ...
        for (int i = 0; i<[titles count]; i++) {
            ...
            [btn setTitleColor:UIColorFromRGBValue(0xa1ceea) forState:UIControlStateNormal];
            [btn setTitleColor:UIColorFromRGBValue(0x11a5e2) forState:UIControlStateSelected];
            ...
            [btn setBackgroundImage:[UIImage imageNamed:@"back_tabitem_mylist"] forState:UIControlStateNormal];
            [btn.titleLabel setFont:[UIFont fontWithName:@"Century Gothic" size:18]];
        ...
        //  lineView
        //
        CGFloat height4Line = HYSegmentedControl_Height / 1.5f;
        ...
        //  bottom lineView
        //
        _bottomLineView = [[UIView alloc] initWithFrame:CGRectMake(0.f, HYSegmentedControl_Height - 4.f, width4btn, 3.0f)];
        _bottomLineView.backgroundColor = UIColorFromRGBValue(0x11a5E2);
...
- (void)segmentedControlChange:(UIButton *)btn
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(hySegmentedControlEnabledSelectAtIndex:)]) {
        if([self.delegate hySegmentedControlEnabledSelectAtIndex:index] == NO)
            return;
    }
...
- (void)setSegmentTitleAtIndex:(NSInteger)index Title:(NSString*)title
{
    if(index < [_array4Btn count]) {
        UIButton *button = [_array4Btn objectAtIndex:index];
